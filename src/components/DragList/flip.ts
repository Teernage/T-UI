export const Flip = (function () {
  ...
  class FlipDom {
    ...
    private firstPosition: { x: number | null; y: number | null } // 元素的初始位置
    private lastPosition: { x: number | null; y: number | null } // 元素的最终位置
    ...
    public play(): Promise<void> {
      return new Promise((resolve) => {
        ...
        this.recordLast()
        const isInvertSucess = this.invert()

        if (isInvertSucess === false) {
          this.reset()
          resolve()
          return
        }
